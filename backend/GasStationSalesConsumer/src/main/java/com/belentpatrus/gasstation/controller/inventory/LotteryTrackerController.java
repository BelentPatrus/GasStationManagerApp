...
public class LotteryTrackerController {
    ...
    @GetMapping("/log/{date}")
    public LotteryTrackerLog getLotteryLog(@PathVariable("date") String date) {
    ...
    public LotteryTrackerLogDailyReportDTO getLotteryLogReport(@PathVariable("date") String date) {
        LocalDate parsedDate = LocalDate.parse(date);
